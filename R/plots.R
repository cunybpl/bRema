...
{ 
  ...
  {
    final_figure = plot_point(df = df1, energy = energy, model_fig = plot_ly(), b_name = b_name)
  }else
  {
    final_figure = main_line_point_plot(df1, best_model, energy, b_name)
  }
  return(final_figure)
}
...
main_line_point_plot <- function(df, best_model, energy, b_name)
{   
	#require(plotly)
